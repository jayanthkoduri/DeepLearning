...
train_context_lengths = [len(context.strip().split()) for context in docs_train]

plt.figure(figsize=(10, 6)) 
plt.hist(train_context_lengths, bins=20, color='skyblue', edgecolor='black')
plt.ylabel('Frequency')
plt.xlabel('Word Count')
plt.title('Distribution of Training Context Lengths')
plt.grid(axis='y', alpha=0.75)  
plt.show()


train_question_lengths = [len(question.strip().split()) for question in queries_train]


plt.figure(figsize=(10, 6))  
plt.hist(train_question_lengths, bins=20, color='salmon', edgecolor='black')
plt.ylabel('Frequency')
plt.xlabel('Word Count')
plt.title('Distribution of Training Question Lengths')
plt.grid(axis='y', alpha=0.75)  
plt.show()
...
transformer_qa_model = TransformerQAModel(transformer_model_name='bert-base-uncased')
...
print('WER( with preprocessing):', wer_scores_over_epochs)
